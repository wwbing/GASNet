...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--data-path', default='./data/pcb.yaml', type=str, help='path of dataset')
    parser.add_argument('--conf-file', default='./config/lvt_v6s_mbla_finetune.py', type=str, help='experiments description file')
    ...
    parser.add_argument('--output-dir', default='./runs/train_local', type=str, help='path to save outputs')
    parser.add_argument('--name', default='neu-det-debug', type=str, help='experiment name, saved to output_dir/name')
    ...
    parser.add_argument('--distill', action='store_true', help='是否蒸馏')
    parser.add_argument('--distill_feat', action='store_true', help='是否蒸馏特征图')
    parser.add_argument('--quant', action='store_true', help='是否量化')
    parser.add_argument('--calib', action='store_true', help='run ptq')
    parser.add_argument('--teacher_model_path', type=str, default=r'C:\Users\jiahao\Desktop\JIAHAO\paper_with_code\project\Gasnet\weights\base\pcb_base.pt', help='teacher model path')
    parser.add_argument('--temperature', type=int, default=30, help='distill temperature')

    parser.add_argument('--fuse_ab', action='store_true', help='fuse ab branch in training process or not 是否启用辅助分支')
    parser.add_argument('--bs_per_gpu', default=4, type=int, help='batch size per GPU for auto-rescale learning rate, set to 16 for P6 models')
...
def check_and_init(args):
    ...
    # 是否要读取权重继续训练：是的话就修改对应的参数，并且save_dir对应上次训练的位置（主要检查有没有resume需要的文件和文件夹）
    if args.resume:
    ...
    # 不继续训练就根据output_dir和name创建新的save_dir
    else:
...
def main(args):
    ...
    # 读取并检查参数，模型配置文件，设备等，写入args.yaml中
