...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.rank = args.rank
        self.main_process = self.rank in [-1, 0]  # True or False
        
        if self.main_process:
            cfg_file_path = osp.join(self.save_dir, 'cfg.txt')
            with open(cfg_file_path, 'w') as f:
                # 获取配置对象的字典表示，然后转换为格式化的字符串
                cfg_dict = self.cfg._cfg_dict
                cfg_str = json.dumps(cfg_dict, indent=4)
                f.write(cfg_str)
        ...
        # 数据集的yaml文件读取信息，后面用于读取dataloader
        self.data_dict = load_yaml(args.data_path)
        self.num_classes = self.data_dict['nc']

        # get model and optimizer
        # 
        self.distill_ns = True if self.args.distill and self.cfg.model.type in ['YOLOv6n','YOLOv6s','YOLOv6s_mbla'] else False

        # 得到模型：backbone neck 蒸馏头，辅助头，加载预训练权重等
        ...
        # 学习率策略，初始化学习率策略后，优化器参数组会多一个initial_lr的字段，保存的最开始的lr
        self.scheduler, self.lf = self.get_lr_scheduler(args, cfg, self.optimizer)
        if self.main_process:
            for i, param_group in enumerate(self.optimizer.param_groups):
                print(f"Parameter Group {i}: initial_lr = {param_group['initial_lr']}")
        ...
        #上面是否加载了resume参数传递的接着训练的模型，如果有，就读取模型内的参数，根据上次训练的模型情况接着进行训练
        ...
        # 得到训练和验证的dataloader,先dataset->dataloader
        ...
        # set color for classnames 随机设置classneme的颜色
    ...
    def train(self):
        try:
            self.before_train_loop()                                                         # 创建损失类
            for self.epoch in range(self.start_epoch, self.max_epoch):
                self.before_epoch()                                                          # 模型转为train模型，优化器梯度归零
    ...
    # Training one batch data.一次迭代
    def train_in_steps(self, epoch_num, step_num):
        # 数据移动到指定的GPU上和图片数据归一化,得到可以输入网络的图片[b,3,h,w] 和targets[n, 6]
        ...
        # forward 模型前向传播
        with amp.autocast(enabled=self.device != 'cpu'):  #自动混合精度（AMP）来加速训练
            _, _, batch_height, batch_width = images.shape

            # 返回的preds包括[[neck的三层特征], cls_score_list, reg_distri_list]    s_featmaps也是neck的三层特征
            ...
            if self.args.distill:
            ...
            elif self.args.fuse_ab:    # 分别计算af和ab部分的损失，然后加起来
                ...
                # 执行self.compute_loss对象的__call__函数
                total_loss, loss_items = self.compute_loss(preds, targets, epoch_num, step_num,
                                                            batch_height, batch_width) # YOLOv6_af
            if self.rank != -1:
                total_loss *= self.world_size

        # backward 反向传播，计算梯度，在后面的update_optimizer()函数种的scaler.step(self.optimizer)中调用optimizer.step()使用该梯度进行模型参数的更新
        self.scaler.scale(total_loss).backward()
        self.loss_items = loss_items
        self.update_optimizer()
    
    # 更新学习率 -> 判断验证与否，执行验证 -> 保存模型 -> tb日志记录信息
    def after_epoch(self):
        lrs_of_this_epoch = [x['lr'] for x in self.optimizer.param_groups]    # 获取当前阶段的学习率,三个参数组的学习率
        self.scheduler.step() # update lr 获取下一epoch的学习率

        if self.main_process:
            self.ema.update_attr(self.model, include=['nc', 'names', 'stride']) # update attributes for ema model

            # 判断是否要验证：如果剩余的训练周期数大于等于 self.args.heavy_eval_range，则验证间隔为 self.args.eval_interval
            # 否则，验证间隔为 self.args.eval_interval 和 3 的较小值
            remaining_epochs = self.max_epoch - 1 - self.epoch 
            eval_interval = self.args.eval_interval if remaining_epochs >= self.args.heavy_eval_range else min(1, self.args.eval_interval)
            ...
            if is_val_epoch:
                self.eval_model()
                self.ap = self.evaluate_results[1]     # result[1]是map0.5:95，这里用的map0.5作为best_ap的衡量标准
                self.best_ap = max(self.ap, self.best_ap)

                # 加入map0.5做记录
                self._ap50 = self.evaluate_results[0]  # result[0]是map0.5
                self.best_ap50 = max(self._ap50, self.best_ap50)
            ...
            save_ckpt_dir_ap50 = osp.join(self.save_dir, 'weights_ap50')

            # 如果该轮的ap50:95有更新，就保存模型
            save_checkpoint(ckpt, (is_val_epoch) and (self.ap == self.best_ap), save_ckpt_dir, model_name='last_ckpt')
            # 如果该轮的ap50有更新，就保存模型
            save_checkpoint(ckpt, (is_val_epoch) and (self._ap50 == self.best_ap50), save_ckpt_dir_ap50, model_name='last_ckpt')
            
            # 如果有指定save_ckpt_on_last_n_epoch，且当前轮满足，直接保存后面所有轮的模型
    ...
    # 模型验证
    def eval_model(self):
        # 判断cfg中是否有eval_params
        # 如果没有，用当前的参数执行eval.py中的run方法
        # run方法返回：# return ( mAP@0.5, mAP@0.5:0.95) , vis_outputs, vis_paths  后两个给tensorboard可视化
        if not hasattr(self.cfg, "eval_params"):
            ...
            # 如果有，就先读取eval_params中的参数，再用读取到的验证的参数执行eval.py中的run方法
    ...
    def before_train_loop(self):
        ...
        # 不量化的基础上，取warmup_epochs*max_stepnum 和 1000的最大值作为warmup_stepnum的值
        self.warmup_stepnum = round(self.cfg.solver.warmup_epochs * self.max_stepnum) if self.args.quant is False else 0
        ...
        # 梯度缩放器用于实现自动混合精度（AMP）训练，scaler(loss)：这个方法接受一个损失值，然后将其乘以一个缩放因子
        self.scaler = amp.GradScaler(enabled=self.device != 'cpu')

        # 加入map0.5作为保存的指标，和记录最好的map0.5，原始只是根据0.5：95做操作
        self.best_ap, self.ap = 0.0, 0.0
        self.best_ap50, self.ap50 = 0.0, 0.0

        self.best_stop_strong_aug_ap = 0.0
        self.evaluate_results = (0, 0) # AP50, AP50_95

        # resume results，如果是接着训练的，就读取上次训练的结果
        if hasattr(self, "ckpt"):
        ...
        # 蒸馏
        if self.args.distill :
            if self.cfg.model.type in ['YOLOv6n','YOLOv6s','YOLOv6s_mbla']:
    ...
    # 模型转换为train模式，并初始化一些参数，如果要停止mosaic增强，会重新加载dataloader
    def before_epoch(self):
        #stop strong aug like mosaic and mixup from last n epoch by recreate dataloader
        # 最后n个阶段通过重新创建dataloader的方式来实现停止mosaic增强
        if self.epoch == self.max_epoch - self.args.stop_aug_last_n_epoch:
            self.cfg.data_aug.mosaic = 0.0
            self.cfg.data_aug.mixup = 0.0
        ...
        self.mean_loss = torch.zeros(self.loss_num, device=self.device) # [3]

        self.optimizer.zero_grad()

        LOGGER.info(('\n' + '%10s' * (self.loss_num + 2)) % (*self.loss_info,))
        
        # 取出每个batch的数据，创建一个迭代器
        self.pbar = enumerate(self.train_loader)
        if self.main_process: # 只有主进程需要显示进度条
            #  tqdm 库创建了一个进度条，可以在终端显示进度条，进度条的总长度，等于最大的步数 self.max_stepnum
    ...
    # 训练结束后，删除模型中删除优化器等相关信息，并保存模型
    def strip_model(self):
        if self.main_process:
            LOGGER.info(f'\nTraining completed in {(time.time() - self.start_time) / 3600:.3f} hours.')
            LOGGER.info(f'Best mAP@0.5: {self.best_ap50:.4f} | Best mAP@0.50:0.95: {self.best_ap:.4f}')
            
            # 将结果写入到result.txt文件中
            result_file_path = osp.join(self.save_dir, 'result.txt')
            with open(result_file_path, 'w') as f:
                f.write(f'Training completed in {(time.time() - self.start_time) / 3600:.3f} hours.\n')
                f.write(f'Best mAP@0.5: {self.best_ap50:.4f} | Best mAP@0.50:0.95: {self.best_ap:.4f}\n')
            
            save_ckpt_dir = osp.join(self.save_dir, 'weights')
            strip_optimizer(save_ckpt_dir, self.epoch)  # strip optimizers for saved pt model

            save_ckpt_dir_ap50 = osp.join(self.save_dir, 'weights_ap50')
            strip_optimizer(save_ckpt_dir_ap50, self.epoch)
    ...
    def update_optimizer(self):
        ...
        self.accumulate = max(1, round(64 / self.batch_size))   # 如果bs过小，可能会累计多次梯度，避免梯度过小，这行代码根据bs动态的计算累计次数

        # 在训练的预热阶段（即前 self.warmup_stepnum 步）中，动态调整优化器的参数
        if curr_step <= self.warmup_stepnum:
            ...
            for k, param in enumerate(self.optimizer.param_groups):
                warmup_bias_lr = self.cfg.solver.warmup_bias_lr if k == 2 else 0.0   # k=2对应第二组的参数，即bias的学习率
        ...
        if curr_step - self.last_opt_step >= self.accumulate:
            self.scaler.step(self.optimizer)        #self.scaler 会调用 self.optimizer.step() 方法，使用当前的梯度来更新模型的参数
            self.scaler.update()                    # 更新梯度缩放器的状态，更新缩放因子
    ...
    def get_data_loader(args, cfg, data_dict):
        ...
        # 32
        grid_size = max(int(max(cfg.model.head.strides)), 32) # max(cfg.model.head.strides) = 32
        ...
        if args.rank in [-1, 0]:
            ...
            # 传入val_path 和 task=='val'，创建 验证集的dataloader
    ...
    # 每个epoch内每次迭代之前数据的预处理
    @staticmethod
    def prepro_data(batch_data, device):
        images = batch_data[0].to(device, non_blocking=True).float() / 255 # 移动到指定设备并归一化
    ...
    def get_model(self, args, cfg, nc, device):
        ...
        if self.main_process:
            LOGGER.info('Backbone: {}'.format(cfg.model.backbone.type))
            LOGGER.info('Neck: {}'.format(cfg.model.neck.type))
            LOGGER.info('Head: {}'.format(cfg.model.head.type))
    ...
    def get_optimizer(self, args, cfg, model):
        accumulate = max(1, round(64 / args.batch_size)) # 计算了累积梯度的次数，多次梯度累计，避免bs太小导致的梯度太小
        cfg.solver.weight_decay *= args.batch_size * accumulate / 64  # 相当于乘以1，不改变weight_decay

        cfg.solver.lr0 *= args.batch_size / (self.world_size * args.bs_per_gpu) # 根据批量大小、GPU 的数量和每个 GPU 的批量大小调整学习率
