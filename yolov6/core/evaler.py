...
class Evaler:
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != 'speed' and len(pred_results):
            ...
            return (map50, map)  # return ( mAP@0.5, mAP@0.5:0.95)
        return (0.0, 0.0)
    ...
    def gradcam_visualization(self, model, weights, image_path, target_layer_name):
        model = self.init_model(model, weights, task='val')
        # 加载模型和数据
        target_layer = dict([*model.named_modules()])[target_layer_name]
        grad_cam = GradCAM(model=model, target_layers=[target_layer], use_cuda=torch.cuda.is_available())

        input_image = self.preprocess_image(image_path, self.img_size)

        # 计算 Grad-CAM
        grayscale_cam = grad_cam(input_tensor=input_image, target_category=None)
