    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                cls_output = self.cls_preds[i](cls_feat)                                # [b, nc, h, w]
                reg_feat = self.reg_convs[i](reg_x)
                reg_output = self.reg_preds[i](reg_feat)                                # [b, 17*4, h, w] 17是dfl的对应数

                cls_output = torch.sigmoid(cls_output)
                cls_score_list.append(cls_output.flatten(2).permute((0, 2, 1)))         # [b, h*w, nc]    
                reg_distri_list.append(reg_output.flatten(2).permute((0, 2, 1)))        # [b, h*w, 17*4]

            cls_score_list = torch.cat(cls_score_list, axis=1)                          # [b, 8400, nc]  8400 = 80*80 + 40*40 + 20*20 三个尺度的特征
            reg_distri_list = torch.cat(reg_distri_list, axis=1)                        # [b, 8400, 17*4]

            return x, cls_score_list, reg_distri_list  # x neck输出的特征
        else:
            cls_score_list = []
            reg_dist_list = []
