...
class RepBiFPANNeck(nn.Module):
    ...
    def __init__(
        ...
        self.reduce_layer0 = ConvBNReLU(
            in_channels=channels_list[4], # 256
        ...
        self.Bifusion0 = BiFusion(
            in_channels=[channels_list[3], channels_list[2]], # 128, 64
            out_channels=channels_list[5], # 64
        )
        self.Rep_p4 = RepBlock(
            in_channels=channels_list[5], # 64
            out_channels=channels_list[5], # 64
            n=num_repeats[5], # 4
            block=block
        )
        ...
        self.Bifusion1 = BiFusion(
            in_channels=[channels_list[2], channels_list[1]], # 64, 32
            out_channels=channels_list[6], # 32
        )

        self.Rep_p3 = RepBlock(
            in_channels=channels_list[6], # 32
            out_channels=channels_list[6], # 32
            n=num_repeats[6],  # 4
            block=block
        )
        ...
        self.downsample2 = ConvBNReLU(
            in_channels=channels_list[6], # 32
            out_channels=channels_list[7], # 32
            kernel_size=3,
            stride=2
        )

        self.Rep_n3 = RepBlock(
            in_channels=channels_list[6] + channels_list[7], # 32 + 32
            out_channels=channels_list[8], # 64
            n=num_repeats[7], # 4
            block=block
        )

        self.downsample1 = ConvBNReLU(
            in_channels=channels_list[8], # 64
        ...
        self.Rep_n4 = RepBlock(
            in_channels=channels_list[5] + channels_list[9], # 64 + 64
            out_channels=channels_list[10], # 128
            n=num_repeats[8],
            block=block
        )
...
from yolov6.models.GD import SimFusion_4in, InjectionMultiSum_Auto_pool
from yolov6.layers.common import ConvBNSiLU

# （GD）增强特征融合后嵌入+通道对齐（ours）
class GD_CSPRepBiFPANNeck(nn.Module):
    """
    CSPRepBiFPANNeck module.
    """
    ...
    def __init__(
        ...
        self.Rep_n4 = stage_block(          # MBLABlock
            in_channels=channels_list[5] + channels_list[9], # 256 + 256
            out_channels=channels_list[10], # 512
            n=num_repeats[8],               # 4
            e=csp_e,                        # 0.5
            block=block                     # Convsilu
        )
        ...
        self.low_IFM = nn.Sequential(
                ConvBNSiLU(in_channels=544,
                     ...
                     kernel_size=1, 
                     stride=1,
                     padding=0),*[RepVGGBlock(in_channels=128, out_channels=128) for _ in range(3)],
                ConvBNSiLU(in_channels=128, out_channels=128, kernel_size=1, stride=1, padding=0),
        )

        # 如果是多卡这里记得设置为SyncBN
        # 本地单卡调试就BN
        self.Inject_bf = InjectionMultiSum_Auto_pool(64,64, norm_cfg=dict(type='BN', requires_grad=True),
    ...
    def forward(self, input):
        ...
        # bf0后注入全局特征
        bf0_out = self.Inject_bf(f_concat_layer0, low_global_info[0])

        f_out0 = self.Rep_p4(bf0_out)

        fpn_out1 = self.reduce_layer1(f_out0)
        f_concat_layer1 = self.Bifusion1([fpn_out1, x2, x3])

        # bf1后注入全局特征
        bf1_out = self.Inject_bf(f_concat_layer1, low_global_info[1])

        pan_out2 = self.Rep_p3(bf1_out)
...
# 通道对齐（但增强特征嵌入）
class CSPRepBiFPANNeck_2(nn.Module):
    """
    CSPRepBiFPANNeck module.
    """
    ...
    def __init__(
        ...
        self.low_FAM = SimFusion_4in()
        self.inj_bf = InjectionMultiSum_Auto_pool(inp=64, oup=64, norm_cfg=dict(type='SyncBN', requires_grad=True),activations=nn.ReLU6,global_inp=544)
    ...
    def forward(self, input):
        ...
        # 增强特征嵌入bf0
        bf0_out = self.inj_bf(f_concat_layer0, low_align_feat)
        ...
        # 增强特征嵌入bf1
        bf1_out = self.inj_bf(f_concat_layer1, low_align_feat)
