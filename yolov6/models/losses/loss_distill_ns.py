...
class ComputeLoss:
    ...
    def __call__(
        ...
        outputs,            # 学生模型的输出
        t_outputs,          # 老师模型的输出
        s_featmaps,         # 学生模型颈部的三层特征
        t_featmaps,         # 老师模型颈部的三层特征
        targets,
        epoch_num,
        max_epoch,
        temperature,
        ...
        # 学生模型的颈部特征，分类预测，分布预测，坐标预测
        feats, pred_scores, pred_distri, pred_lrtb = outputs
        t_feats, t_pred_scores, t_pred_distri = t_outputs[0], t_outputs[-2], t_outputs[-1]  # 拿出老师模型的颈部特征，分类分数，af分支的分布输出
        ...
        pred_bboxes = self.bbox_decode(anchor_points_s, pred_distri) #xyxy #distri branch    把分布转换为坐标
        pred_bboxes_lrtb = dist2bbox(pred_lrtb, anchor_points_s) #iou branch    # 坐标本身的变换 xyxy - xywh
        ...
        # loss_iou：分布转坐标再和target计算iou + 坐标和target计算iou
        # loss_dfl:本身的dfl损失
        # d_loss_dfl:蒸馏的dfl损失
        loss_iou, loss_dfl, d_loss_dfl = self.bbox_loss(pred_distri,                # 学生模型的分布输出
                                                        pred_bboxes_lrtb,           # 学生模型的坐标输出
                                                        pred_bboxes,                # 学生模型分布输出变换后的坐标输出
                                                        t_pred_distri,              # 老师模型的分布输出
                                                        t_pred_bboxes,              # 老师模型分布输出变换后的坐标输出
                                                        temperature,
                                                        anchor_points_s,
                                                        target_bboxes,
                                                        target_scores,
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist,                    # x学生模型的分布输出
                      pred_bboxes_lrtb,             # 学生模型的坐标输出
                      pred_bboxes,                  # 学生模型分布输出变换后的坐标输出
                      t_pred_dist,                  # 老师模型的分布输出
                      t_pred_bboxes,                # 老师模型分布输出变换后的坐标输出
                      temperature,
                      anchor_points,
                      target_bboxes,                # 目标框
