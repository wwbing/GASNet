...
class MBLABlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n=1, e=0.5, block=RepVGGBlock):
        ...
        if n == 1:
            ...
            n_list = [0, extra_branch_steps, n]   # 传入的n为4，这里的n_list为[0, 1, 2]

        branch_num = len(n_list) # 3

        c_ = int(out_channels * e)  # hidden channels: 128
        self.c = c_
        ...
        if block == ConvBNSiLU:
            self.cv1 = ConvModule(in_channels, branch_num * self.c, 1, 1, 'silu', bias=False)  # # 192 -> 384
            self.cv2 = ConvModule((sum(n_list) + branch_num) * self.c, out_channels, 1, 1,'silu', bias=False)  # # 768 -> 256
        ...
        self.split_num = tuple([self.c]*branch_num)   # 128,128,128

    def forward(self, x):
        y = list(self.cv1(x).split(self.split_num, 1)) # 384 -> 128, 128, 128

        all_y = [y[0]]      # 复制y[0]进一个新的列表
